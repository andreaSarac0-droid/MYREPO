({
	...
	alert: function(component, event, helper, variant, title, message){
     ...
     buttonFilter: function (component,event,helper,recordId){
        ...
        var action = component.get("c.filterDocuSignButton");
        action.setParams({
            "recordId" : recordId
        })
    ...
    invokeResendLink: function(component, event, helper, recordId){
        ...
        invokeHideResendLink: function(component, event, helper, recordId){
            
            var action2 = component.get("c.hideResendLink");
            action2.setParams({
                "recordId" : recordId
            })
            action2.setCallback(this, function (response) {
                ...
                if (state === "SUCCESS") {
                    if (result != null) {
                        console.log('result hideResendLink', result);   
                        component.set("v.hideResendLink", result);
                    } 
                } else if (state === "ERROR") {
                    //helper.alert(component, event, helper, 'error', 'ERROR', 'Please, contact your system administrator.');
                }
